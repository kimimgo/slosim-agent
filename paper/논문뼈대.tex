...
10개 시나리오에서 M-A3 파라미터 정확도 69.5\%를 달성하고(32B$\approx$8B, $\Delta$=5.0\%pp),
...
\item \textbf{SPH 솔버 에이전트 부재} --- 7종 에이전트 전부 OpenFOAM(격자 FVM) only $\leftarrow$ EXP-C + Bridge
...
\subsection{Abstract}
Gap 3개 $\rightarrow$ SlosimAgent 제안 $\rightarrow$ 10 시나리오 M-A3 69.5\% (proof-of-concept) $\rightarrow$ 2$\times$2 ablation (80 runs): hierarchical dependency (Prompt=prerequisite, Tool=+20.9\%pp interaction) $\rightarrow$ 32B$\approx$8B (9/10 identical, $\Delta$=5.0\%pp) $\rightarrow$ SPHERIC T10 정량 검증 + Bridge 5/5 PASS
...
\begin{itemize}[nosep,leftmargin=1.5em]
\item P1(motion\_type: sway 전용) + P2(amplitude: meters 전용) 확장 시: M-A3 69.5\% $\rightarrow$ \textbf{$\sim$75--80\%}
...
\begin{itemize}[nosep,leftmargin=1.5em]
\item 32B$\approx$8B: 10개 시나리오 중 \textbf{9개 동일} (S08만 예외: 32B=50\%$>$8B=0\% --- 8B tool calling 실패)
\item 전체 $\Delta$=5.0\%pp --- 아키텍처 효과(+56.5\%pp)의 1/11 수준
...
\begin{itemize}[nosep,leftmargin=1.5em]
\item \textbf{성능}: M-A3 69.5\% (proof-of-concept). 도구 커버리지 확장 시 $\sim$80\%
...
\item \textbf{Self-verification 부재}: {\color{red}\textbf{[Gemini 반영]}} 에이전트가 자신의 출력 오류를 감지하는 메커니즘 없음. Iterative refinement (EXP-D: 에러 $\rightarrow$ 수정 루프) 구현이 다음 단계
\item S08: 운동 파라미터를 명시하지 않은 implicit knowledge test $\rightarrow$ 8B 실패는 암묵적 추론 한계
\end{itemize}

\subsection{7. Conclusion}
\begin{enumerate}[nosep,leftmargin=1.5em]
\item 최초 비전문가용 SPH 슬로싱 AI 에이전트 --- M-A3 69.5\% (proof-of-concept), 10 시나리오 60 runs, Bridge 5/5 physics PASS
\item 2$\times$2 ablation (80 runs)으로 hierarchical dependency 발견:
  ...
  \item 도구 커버리지가 성능 천장 (69.5\%$\rightarrow$$\sim$80\% 확장 가능)
  \end{itemize}
...
Table 7 & 체계적 오류 패턴 P1--P5 (빈도, 영향 시나리오) & \textbf{신규} \\
\midrule
...
\begin{quote}
\textit{To our knowledge, SlosimAgent is the first AI agent enabling non-expert users to configure, execute, and analyze SPH sloshing simulations through natural language. We contribute: (1) a domain-specialized agent integrating DualSPHysics GPU with a local SLM (Qwen3-32B/8B) via 18 tools --- the first LLM agent for particle-based CFD, achieving M-A3 69.5\% across 10 sloshing scenarios as a proof-of-concept; (2) a 2$\times$2 factorial ablation (80 runs: 10 scenarios $\times$ 4 conditions $\times$ 2 models) revealing a \textbf{hierarchical dependency}: the domain prompt is a necessary condition for tool usage (B1=B4=0\%, 40/40 runs), and tools provide +20.9\%pp conditional refinement --- with tool design coverage, not model scale, determining the performance ceiling (M-A3 69.5\%$\rightarrow$$\sim$80\% via tool extension); (3) empirical evidence that 32B$\approx$8B ($\Delta$=5.0\%pp, 9/10 scenarios identical), enabling local deployment without cloud dependency; and (4) quantitative validation against SPHERIC Test 10 (M2=19.5\%, r=0.655) with a proposed M1--M8 framework, plus a Bridge experiment confirming agent-generated XML produces physically valid simulations (5/5 checks PASS).}
\end{quote}
