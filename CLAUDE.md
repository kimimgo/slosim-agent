...
├── research-v2             ← EXP 수행 이력 v2 (진행중)
├── research-v3, ...        ← 향후 실험 버전
└── paper                   ← 논문 작성 (항상 최신 뼈대 유지)
```
...
| `paper` | 논문 자료 통합 | sniper 서베이, 분석 스크립트, 그래프, 초고, 실험 데이터 | 대용량 바이너리 |

### 데이터 흐름

```
research-vN (실험 완료) ──→ paper (분석·그래프·메트릭 cherry-pick)
main (구현 완료)        ──→ paper (에이전트 아키텍처 설명 참조)
```
