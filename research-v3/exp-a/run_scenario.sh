...
# Ollama 로컬 프로바이더 설정 (slosim-agent 필수)
...
# 생성된 XML 및 시뮬레이션 결과 보존
if [ -d "$SIMDIR" ] && [ "$(ls -A "$SIMDIR" 2>/dev/null)" ]; then
    cp -r "$SIMDIR" "${OUTDIR}/simulations"
    echo "[$(date +%H:%M:%S)] Saved simulation artifacts to ${OUTDIR}/simulations/"
fi
# 생성된 XML 파일 별도 복사 (분석 편의)
