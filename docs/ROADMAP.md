...
| **v0.3** | TUI 디자인 혁신 + 프로덕션 안정성 | Design Token + Widget, huh Wizard, K9s/lazygit 참조, 에이전트 E2E | PLANNED |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
자연어 입력 ✅      ───→  이미지 결과 확인 ✅  ───→  huh Case Wizard   ───→  케이스 간 비교
텍스트 리포트 ✅     ───→  Mesa 렌더링 ✅      ───→  K9s Sim Dashboard ───→  비교 리포트
...
스텁 TUI ✅         ───→  Theme 48 methods ✅ ───→  Design Token+Widget ─→  Parametric View
                         GPU 10시나리오 ✅
```
...
## v0.3 — TUI 디자인 혁신 + 에이전트 통합 E2E + 프로덕션 안정성

**테마:** "보기 좋은 TUI" + 사용자 친화 UX + 에이전트 호출 E2E 검증
**예상 기간:** 4주
**의존 근거:** v0.2에서 18개 도구와 GPU 파이프라인은 검증됨. TUI 스텁 컴포넌트 리디자인 + 에이전트 통합 E2E 필요.
**디자인 상세:** `docs/TUI_DESIGN_PLAN.md`

### TUI 디자인 전략 (AI 한계 극복)

| 전략 | 설명 |
|------|------|
| **Design Token 시스템** | `theme.Tokens().XXX` 강제 — AI 임의 색상 차단 |
| **Reference-Driven** | K9s, lazygit, gum, soft-serve 참조 지정 |
| **Component-First** | Bubbles (viewport/table/list/progress) 우선 사용 |
| **huh 폼 라이브러리** | Case Wizard용 단계별 입력 (Catppuccin 테마) |
| **harmonica 애니메이션** | 스프링 기반 부드러운 진행률 바 |
| **VHS 스크린샷 검증** | 시각적 회귀 자동 감지 |

### 포함 기능

| ID | 기능명 | 설명 | 전제 |
|----|--------|------|------|
| **TUI-DS** | **Design Token + Widget 라이브러리** | SemanticTokens, Panel, StatusBadge, Metric, KeyHint | theme ✅ |
| **TUI-04** | **Case Wizard** | huh 기반 4단계 대화형 조건 설정 UI | TUI-DS |
| **TUI-05** | **Sim Dashboard 리디자인** | K9s 참조, Bubbles table+progress+viewport, 실시간 메트릭 | TUI-DS, MON-01 ✅ |
| **TUI-07** | **Results Browser 리디자인** | lazygit 참조, 2-패널 (목록+미리보기), 포커스 상태 | TUI-DS |
| **TUI-08** | **Error Panel 리디자인** | 인라인 에러 + 원클릭 복구 액션 | TUI-DS, NFR-01 ✅ |
| **TUI-09** | **Log Viewer 고도화** | 필터/검색, 레벨별 색상, vim 키바인딩 | TUI-DS |
...
```
Week 1: TUI 인프라
├── TUI-DS: SemanticTokens + widgets/ (Panel, StatusBadge, Metric, KeyHint)
├── go.mod: huh, harmonica 추가
├── CLAUDE.md: TUI 코딩 규칙 추가
└── AGENT-E2E: 에이전트 통합 E2E (병렬)

Week 2: TUI 컴포넌트 리디자인
├── TUI-05: Sim Dashboard (K9s 참조, Bubbles table+progress+viewport)
├── TUI-07: Results Browser (lazygit 참조, 2-패널)
└── QWEN-8B: 8B 모델 프롬프트 검증 (병렬)

Week 3: 신규 컴포넌트 + 도메인 검증
├── TUI-04: Case Wizard (huh 4단계)
├── TUI-08: Error Panel + TUI-09: Log Viewer
└── MDBC-01 + GEO-02/EXC-E2E (GPU 검증, 병렬)

Week 4: 통합 QA
├── 10개 테마 순회 테스트
├── 터미널 크기 반응형 테스트 (80x24 ~ 200x60)
├── VHS 스크린샷 자동 생성
└── JOB-02 + 전체 통합 QA
```
...
| 테마 호환 | 10개 테마 `Ctrl+T` 전환 시 **깨지지 않음** |
| 반응형 레이아웃 | **80x24** 최소 크기에서 모든 컴포넌트 렌더 가능 |
| Bubbles 사용 | 커스텀 스크롤/테이블/진행률 → **0건** (Bubbles 필수) |
| Widget 사용 | 새 패널 → `widgets.Panel()` 사용 필수 |
...
| TUI-* | BubbleTea TUI 컴포넌트 | v0.1 ~ v1.0 (DS=Design System, 04=Wizard, 05=Dashboard, 07=Results, 08=Error, 09=Logs) |
